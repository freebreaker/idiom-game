...
import { checkIsHidden, getQueryString, getRandomEasyRecord, getRandomWord, getWordContainArr, MD5, wordGenerate } from '../../../tools/game'
...
let exampleOne = [["", "", "", "", "", "人", "", "", "", "", "", "", "", "", "", ""], ["", "", "", "", "漫", "[山]", "遍", "[野]", "", "", "", "", "", "", "", ""], ["", "", "", "", "", "人", "", "心", "", "", "", "", "", "", "", ""], ["", "", "", "", "", "海", "", "[勃]", "然", "大", "[怒]", "", "", "", "", ""], ["", "", "", "", "", "", "", "勃", "", "", "", "", "", "", "", ""], ["", "", "", "", "", "", "", "", "", "", "", "", "", "", "", ""]]
let exampleOneString = '人山人海,漫山遍野,野心勃勃,勃然大怒'
...
const Game: NextPage = (props) => {
  ...
  const submitOne = async (matrix: string[][], id?: string) => {
    ...
    const res = await axios.post('/add', {
      ...
      sort: sort,
      name: `第${sort}关`,
      matrix: sort === 1 ? JSON.stringify(exampleOne) : JSON.stringify(matrix),
      matrix_space: sort === 1 ? exampleOneString : matrixRecord.toString(),
    })
    if (id && res) {
  ...
  const submitJson = async () => {
    ...
    const { matrix, matrix_space } = getRandomEasyRecord(sort)
    const res = await axios.post('/add', {
      level: easy,
      sort: sort,
      name: `第${sort}关`,
    ...
    <div className={styles.container}>
      ...
      >
        ...
        <Form.Item wrapperCol={{ offset: 8, span: 16 }}>
          ...
          <Button onClick={() => {
            initData = createData(xBlockNum, yBlockNum)
            const { matrix, matrix_space } = getRandomEasyRecord(1, true)
            initData = matrix
            setData(matrix)
