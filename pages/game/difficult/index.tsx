...
import { checkIsHidden, getQueryString, getRandomDifficultRecord, getRandomWord, getWordContainArr, MD5, wordGenerate } from '../../../tools/game'
...
const Game: NextPage = (props) => {
  const easy = 3
  const key = 'update'
  const [idiomCount, setIdiomCount] = useState(8)
  ...
  const submitJson = async () => {
    ...
    const { matrix, matrix_space } = getRandomDifficultRecord(sort)
    const res = await axios.post('/add', {
      level: easy,
      sort: sort,
      name: `第${sort}关`,
    ...
    <div className={styles.container}>
      ...
      >
        ...
        <Form.Item wrapperCol={{ offset: 8, span: 16 }}>
          ...
          <Button onClick={() => {
            initData = createData(xBlockNum, yBlockNum)
            const { matrix, matrix_space } = getRandomDifficultRecord(1, true)
            initData = matrix
            setData(matrix)
